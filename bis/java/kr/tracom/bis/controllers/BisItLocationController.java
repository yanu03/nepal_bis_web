...
import kr.tracom.bis.domain.bisItLocation.BisItLocation;
import kr.tracom.bis.domain.bisItLocation.BisItLocationService;
...
@Controller
@RequestMapping(value = "/api/v1/bisItLocations")
public class BisItLocationController extends BaseController {

	@Inject
	private BisItLocationService bisItLocationService;

	@RequestMapping(method = RequestMethod.GET, produces = APPLICATION_JSON)
	public List<BisItLocation> list(RequestParams requestParams, HttpServletRequest request) {
		// List list = bisMtRouteService.gets(requestParams);
		// return Responses.ListResponse.of(list);

		Map parameterMap = RequestUtil.getParameterMap(request);
		List<BisItLocation> list = null;
		list = bisItLocationService.findAll(parameterMap);
