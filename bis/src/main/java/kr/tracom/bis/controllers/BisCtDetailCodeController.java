...
@Controller
@RequestMapping(value = "/api/v1/bisMtDetailCodes")
public class BisCtDetailCodeController extends BaseController {

	@Inject
	private BisCtDetailCodeService bisCtDetailCodeService;

	@RequestMapping(method = RequestMethod.GET, produces = APPLICATION_JSON)
	public List<BisCtDetailCode> list(RequestParams requestParams, HttpServletRequest request) {
		// List list = bisMtRouteService.gets(requestParams);
		// return Responses.ListResponse.of(list);

		Map parameterMap = RequestUtil.getParameterMap(request);
		List<BisCtDetailCode> list = null;
		list = bisCtDetailCodeService.findAll(parameterMap);
		return list;
	}
